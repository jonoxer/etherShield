  ...
  if (plen != 0) {
    ...
    if (buf[IP_PROTO_P] == IP_PROTO_TCP_V && buf[TCP_DST_PORT_H_P] == 0 && buf[TCP_DST_PORT_L_P] == mywwwport) {
      ...
      if (buf[TCP_FLAGS_P] & TCP_FLAGS_ACK_V) {
        ...
        plen = print_webpage(buf, on_off);
        plen = print_webpage(buf, on_off); // Why is this line in here twice? Surely this is a bug
...
uint16_t print_webpage (uint8_t *buf, byte on_off)
{
  ...
  plen = es.ES_fill_tcp_data_p (buf, plen, PSTR("</center><hr> <p> V1.1 <a href=\"http://www.nuelectronics.com\">www.nuelectronics.com<a>"));
